...
HydrostaticEquilibriumBox<GravitationalField, EntropyProfile>::
  ...
  R_fix_ =
      pin->GetOrAddReal("problem/cluster", "R_fix", 1953.9724519818478 * units.kpc());
  ...
  if (test_he_sphere) {
    const Real test_he_sphere_R_start = pin->GetOrAddReal(
        "problem/cluster", "test_he_sphere_R_start_kpc", 1e-3 * units.kpc());
    const Real test_he_sphere_R_end = pin->GetOrAddReal(
        "problem/cluster", "test_he_sphere_R_end_kpc", 4000 * units.kpc());
    const int test_he_sphere_n_r =
        pin->GetOrAddInteger("problem/cluster", "test_he_sphere_n_r", 4000);
...
template class HydrostaticEquilibriumBox<ClusterGravity, cluster::ACCEPTEntropyProfile>;
...
template HydrostaticEquilibriumBox<ClusterGravity, cluster::ACCEPTEntropyProfile>::PRhoProfile<
    Kokkos::View<parthenon::Real *, parthenon::LayoutWrapper, parthenon::DevMemSpace>>
    HydrostaticEquilibriumBox<ClusterGravity, cluster::ACCEPTEntropyProfile>::
